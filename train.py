...
def export_model_weights(model: DecoderTransformer):
    ...
    for h in range(config.NUM_HEADS):
        save_memory_file(quantize_weight(cast(nn.Linear, model.attention.q_heads[h]).weight.T.contiguous(), 16), f"memory/attention_wq_head{h}.mem", 16)
        save_memory_file(quantize_weight(cast(nn.Linear, model.attention.k_heads[h]).weight.T.contiguous(), 16), f"memory/attention_wk_head{h}.mem", 16)
        save_memory_file(quantize_weight(cast(nn.Linear, model.attention.v_heads[h]).weight.T.contiguous(), 16), f"memory/attention_wv_head{h}.mem", 16)
    save_memory_file(quantize_weight(model.attention.output_projection.weight.T.contiguous(), 16), "memory/attention_wo.mem", 16)
    ...
    save_memory_file(quantize_weight(cast(torch.Tensor, model.ffn[0].weight).T.contiguous(), 16), "memory/ffn_w1.mem", 16)
    save_memory_file(quantize_weight(cast(torch.Tensor, model.ffn[0].bias), 16), "memory/ffn_b1.mem", 16)
    save_memory_file(quantize_weight(cast(torch.Tensor, model.ffn[2].weight).T.contiguous(), 16), "memory/ffn_w2.mem", 16)
